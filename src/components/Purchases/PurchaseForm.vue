<template>
  <div>
    <v-card style="overflow-y: auto;">
      <v-card-title>
        <span class="text-h5" v-if="isNewTender">Новые закупки</span>
        <span class="text-h5" v-else>Закупки</span>
      </v-card-title>
      <v-card-text>
            <v-container>
              <v-form ref="form2">
                <v-row>
                  <v-col cols="12" md="6">
                    <v-text-field v-model="date" :rules="isValidDate" label="Дата"
                      hint="Дата в формате дд.мм.гггг — 02.06.2023" persistent-hint></v-text-field>
                  </v-col>
                  <v-col cols="12" md="6">
                    <v-text-field v-model="amount" label="Сумма"
                  ...
                  <v-col cols="12">
                    <div>
                      ...
                      <v-btn color="primary" title="Добавить" @click="addEquipment" v-if="access.purchases > 1">
                    ...
                    <template v-for="item in equipment">
                      <div class="pa-2 d-flex align-center justify-space-between">
                        ...
                        <div class="d-flex ml-4" style="margin-left: auto;">
                          <v-btn v-if="access.purchases > 1" @click="editEqipment(equipment.indexOf(item), item)"
                            color="primary" title="Изменить"><v-icon icon="mdi-pencil"></v-icon></v-btn>
                          <v-btn v-if="access.purchases > 2" @click="deleteEqipment(equipment.indexOf(item))"
                  ...
                  <v-col cols="12">
                    ...
                    <div class="d-flex align-center justify-space-between">
                      <div class="d-flex">
                        <span class="mr-4 line-title">Документы</span>
                      </div>
                      <v-btn color="primary" title="Добавить файл"
                        @click="addNewPhoto('Документы', 'documents')"
                        v-if="access.purchases > 1 && !isNewTender">
                        <v-icon size="large" icon="mdi-plus"></v-icon>
                      </v-btn>
                    </div>
                    <div v-if="documents">
                        ...
                        <template v-for="file in documents">
                          <template v-if="isImage(file)">
                            <v-col cols="12" md="6" lg="4" xl="3">
                              <div class="d-flex">
                                <img :src="file.name" width="40" height="40" class="mr-4 image-mini" @click="openWatchImage(`${mediaContentPath}${id}/documents/${file.name}?as_photo=true`)" title="Посмотреть фото">
                                <span class="mr-3 media-file__name" :title="file.name"> {{ file.name }} </span>
                                <div class="d-flex" style="margin-left:auto;">
                                  <v-btn class="mr-3" color="primary" size="small" title="Посмотреть фото"  @click="openWatchImage(`${mediaContentPath}${id}/documents/${file.name}?as_photo=true`)">
                                    <v-icon icon="mdi-eye"></v-icon>
                                  </v-btn>
                                  <v-btn :download="file.name" :href="`${mediaContentPath}${id}/documents/${file.name}`" color="add-new" size="small" title="Скачать файл">
                                  ...
                                  <v-btn 
                                    ...
                                    size="small"
                                    title="Удалить файл"
                                    @click="deleteFile(file.id, 'documents')"
                                  >
      ...
      <v-card-actions>
        ...
        <v-btn color="primary" @click="saveStage" v-if="isNewTender && access.purchases > 1">
          Создать
        </v-btn>
        <v-btn color="primary" @click="saveStage" v-if="!isNewTender && access.purchases > 1">
          Сохранить
...
import TenderEquipmentForm from '@/components/Purchases/TenderEquipmentForm.vue'
...
export default {
  ...
  async mounted() {
    if (Object.keys(this.tender).length > 0) {
      ...
      console.log(this.tender.tender)
      this.date = formatToDate(this.tender.tender.date)
      this.amount = this.tender.tender.amount
      this.equipment = [...this.tender.tender.equipment]
    } else {
    ...
    this.documents = await this.getFolderMedia(this.id, 'documents')
  },
  ...
  data: () => ({
    ...
    isUploading: false,
    isFileLoading: false,
    mediaContentPath: `api/files/path/media/purchases/`,
    documents: [],
  ...
  methods: {
    ...
    saveStage() {
      const tender = {
        ...
        date: formatFromDate(this.date),
        equipment: this.formatTypeEquipment(this.equipment),
      }
    ...
    addEquipment() {
      this.equipment.push({ name: 'Новое оборудование', price: '', count: '', variation: '' })
    },
    ...
    saveCurrentEqipment(item) {
      this.equipment[item.id] = item.value
      this.dialogEquipment = false
    }
  }
}
