<template>
  <div
    v-if="access.keys > 1"
    style="padding-left: 20px; padding-top: 100px; padding-right: 20px"
  >
    <div>
      <div
        class="text-h5 font-weight-bold mb-10"
        v-text="
          'Для генерации ключа введите запрос и нажмите кнопку &quot;Создать&quot;'
        "
      />
      ...
      <v-row class="pt-3 pb-7 mt-4 checkbox-row">
        <v-col
          class="pb-0 col-12 col-sm-6 col-md-4 col-lg-3"
          cols="12"
          sm="4"
          md="3"
        >
    ...
    <div
      v-if="isAlertVisible"
      class="overflow position-fixed"
    >
      <v-alert
        class="alert"
        densemdi-logout
        text
        :type="isFetchSuccess ? 'add-new' : 'error'"
      >
        {{
          isUnautorized
            ? "Неверный пароль!"
            : isFetchSuccess
              ? "Ключ успешно сгенерирован и скопирован в буфер обмена!"
...
export default {
  ...
  methods: {
    ...
    async submit(e) {
      ...
      const noncePropery = this.nonces
        .map((nonce) => nonce.replace(/\s/g, ''))
        .join(',')

      const response = await fetch(
        `${import.meta.env.VITE_VUE_APP_SERVER}api/key?nonce=${noncePropery}&regcount=${this.regcount}&options=${this.options}&company=${this.customer}&reason=${this.additionalInfo}`,
      )
